...
u.UserRoleId equals r.UserRoleId
       ...
       on u.UserRoleId equals ur.UserRoleId
        ...
        //Buyer and Saller Logins

        [HttpGet("{MobileNo}/{RoleId}")]
        public IActionResult GetUserDetailsByMobileNo(string MobileNo, int RoleId)
        {
            var a = (from u in _context.Users
                     select new
                     {
                         u.UserId,
                         u.UserName,
                     ...
                     ).Where(x => x.UserName == MobileNo && x.IsActive == true && x.UserRoleId == RoleId).ToList();
            return Ok(new { data = a });
        }



        [HttpGet("{MobileNo}/{Password}/{RoleId}")]
        public IActionResult LoginUsingMobileNoAndPasswordAndRole(string MobileNo,string Password, int RoleId)
        {
            var a = (from u in _context.Users
                     select new
                     {
                     ...
                     ).Where(x => x.UserName == MobileNo && x.IsActive == true && x.UserRoleId == RoleId && x.Password ==  Password).ToList();
            return Ok(new { data = a });
        }


        [HttpGet("{MobileNo}/{Password}")]
        public IActionResult LoginUsingMobileNoAndPassword(string MobileNo, string Password)
        {
            var a = (from u in _context.Users
                     select new
                     {
                     ...
                     ).Where(x => x.UserName == MobileNo && x.IsActive == true  && x.Password == Password).ToList();
            return Ok(new { data = a });
        }

        [HttpPost]
        public IActionResult RegisterUser(UserRegistrationDTO userRegistrationDTO)
        {
            var objCheck = _context.Users.SingleOrDefault(opt => opt.UserName == userRegistrationDTO.Mobile && opt.UserRoleId == userRegistrationDTO.UserRoleId);
            if (objCheck == null)
            {
